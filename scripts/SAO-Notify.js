function SAONotify(title,message,action){
  ...
  if (action){
    var template =`<div id="SAO-Notify"><style>` + tempstyle +`</style><div class="notify-title">` + `${title}` + `</div><div class="notify-alert"> `+ `${message}` + `</div><div class="notify-button"><span class="notify-confirm"><button class="far fa-circle" type="button" name="confirm" onclick="clickAudio();setTimeout(function(){` + `${action}` + `},500);cancelNotify()"></button></span><span class="notify-cancel"><button class="fa fa-times" type="button" name="cancel" onclick="panelAudio();cancelNotify()"></button></span></div><audio id="SAO-Notify-Panel" src="https://cdn.refrain.ga/gh/nova1751/sources/audio/Panel.mp3"></audio><audio id="SAO-Notify-Click" src="https://cdn.refrain.ga/gh/nova1751/sources/audio/Click.mp3"></audio>
    </div>`
  } else { //若未定义action代码片段，则仅输出单选项的弹窗
    var template =`<div id="SAO-Notify"><style>` + tempstyle +`</style><div class="notify-title">` + `${title}` + `</div><div class="notify-alert"> `+ `${message}` + `</div><div class="notify-button"><span class="notify-receive"><button class="fas fa-check" type="button" name="receive" onclick="panelAudio();cancelNotify()"></button></span></div><audio id="SAO-Notify-Panel" src="https://cdn.refrain.ga/gh/nova1751/sources/audio/Panel.mp3"></audio><audio id="SAO-Notify-Click" src="https://cdn.refrain.ga/gh/nova1751/sources/Click.mp3"></audio>
    </div>`
  };
